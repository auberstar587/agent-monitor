...
function formatActivity(
  ...
  switch (entry.action) {
    case "created":
      return tr("issues.createdIssue");
    case "status_changed":
      return tr("issues.changedStatus", { from: statusLabel(details.from ?? "?"), to: statusLabel(details.to ?? "?") });
    case "priority_changed":
      return tr("issues.changedPriority", { from: priorityLabel(details.from ?? "?"), to: priorityLabel(details.to ?? "?") });
    case "assignee_changed": {
      const isSelfAssign = details.to_type === entry.actor_type && details.to_id === entry.actor_id;
      if (isSelfAssign) return tr("issues.selfAssigned");
      const toName = details.to_id && details.to_type && resolveActorName
        ? resolveActorName(details.to_type, details.to_id)
        : null;
      if (toName) return tr("issues.assignedTo", { name: toName });
      if (details.from_id && !details.to_id) return tr("issues.removedAssignee");
      return tr("issues.changedAssignee");
    }
    case "due_date_changed": {
      if (!details.to) return tr("issues.removedDueDate");
      const formatted = new Date(details.to).toLocaleDateString("en-US", { month: "short", day: "numeric" });
      return tr("issues.setDueDate", { date: formatted });
    }
    case "title_changed":
      return tr("issues.renamedIssue", { from: details.from ?? "?", to: details.to ?? "?" });
    case "description_updated":
      return tr("issues.updatedDescription");
    case "task_completed":
      return tr("issues.completedTask");
    case "task_failed":
      return tr("issues.taskFailed");
    default:
      return entry.action ?? "";
  }
}
      ...
      <div className="flex h-full flex-col">
        <PageHeader className="gap-2 bg-background text-sm">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Tooltip>
              ...
              />
              <TooltipContent side="bottom">{t("issue.detail.toggleSidebar")}</TooltipContent>
            </Tooltip>
          </div>
        </PageHeader>
        ...
        <div className="mx-auto w-full max-w-4xl px-8 py-8">
          ...
          {parentIssue && (
            ...
            >
              <span className="font-medium shrink-0">{t("issue.detail.subIssueOf")}</span>
              <StatusIcon status={parentIssue.status} className="h-3.5 w-3.5 shrink-0" />
          ...
          {childIssues.length === 0 && (
            <div className="mt-6">
              ...
              >
                ...
                <span>{t("issue.detail.addSubIssues")}</span>
              </button>
            </div>
          )}
          {childIssues.length > 0 && (() => {
              ...
              <div className="mt-10">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  >
                    ...
                    />
                    <span>{t("issue.detail.subIssues")}</span>
                  </button>
                  ...
                  <Tooltip>
                    ...
                    />
                    <TooltipContent side="bottom">{t("issue.detail.addSubIssue")}</TooltipContent>
                  </Tooltip>
                </div>
          ...
          <div>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <h2 className="text-base font-semibold">{t("issue.detail.activity")}</h2>
              </div>
                  ...
                  <PopoverContent align="end" className="w-64 p-0">
                    <Command>
                      ...
                      <CommandList className="max-h-64">
                        <CommandEmpty>{t("issue.detail.noResultsFound")}</CommandEmpty>
                        {members.length > 0 && (
