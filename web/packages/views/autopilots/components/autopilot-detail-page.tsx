...
const RUN_STATUS_CONFIG: Record<string, { key: string; color: string; icon: typeof CheckCircle2; spin?: boolean }> = {
  issue_created: { key: "autopilot.detail.issueCreated", color: "text-blue-500", icon: Clock },
  running: { key: "autopilot.detail.running", color: "text-blue-500", icon: Loader2, spin: true },
  completed: { key: "autopilot.detail.completed", color: "text-emerald-500", icon: CheckCircle2 },
  failed: { key: "autopilot.detail.failed", color: "text-destructive", icon: XCircle },
};

function RunRow({ run, t }: { run: AutopilotRun; t: (key: string, params?: Record<string, string>) => string }) {
  ...
  const content = (
    <>
      ...
      <span className={cn("w-24 shrink-0 text-xs font-medium", cfg.color)}>{t(cfg.key)}</span>
      <span className="w-16 shrink-0 text-xs text-muted-foreground capitalize">{run.source}</span>
      <span className="flex-1 min-w-0 text-xs text-muted-foreground truncate">
        {run.issue_id ? (
          t("autopilot.detail.issueLinked")
        ) : run.failure_reason ? (
...
function TriggerRow({ trigger, autopilotId, t }: { trigger: AutopilotTrigger; autopilotId: string; t: (key: string, params?: Record<string, string>) => string }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t("autopilot.detail.triggerDeleted"));
      setConfirmOpen(false);
    } catch {
      toast.error(t("autopilot.detail.failedDeleteTrigger"));
    } finally {
      setDeleting(false);
    }
  };
    ...
    <div className="flex items-center gap-3 rounded-md border px-3 py-2">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          ...
          {!trigger.enabled && (
            <span className="text-xs bg-muted px-1.5 py-0.5 rounded">{t("autopilot.detail.disabled")}</span>
          )}
        </div>
        ...
        {trigger.next_run_at && (
          <div className="text-xs text-muted-foreground">
            {t("autopilot.detail.next", { date: formatDate(trigger.next_run_at) })}
          </div>
        )}
      </div>
      <Button
      ...
      <AlertDialog open={confirmOpen} onOpenChange={(v) => { if (!v && !deleting) setConfirmOpen(false); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("autopilot.detail.deleteTrigger")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("autopilot.detail.deleteTriggerDesc")}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting}>{t("autopilot.detail.cancelTrigger")}</AlertDialogCancel>
            <AlertDialogAction
              onClick={handleDelete}
              disabled={deleting}
              className="bg-destructive text-white hover:bg-destructive/90"
            >
              {deleting ? t("autopilot.detail.deleting") : t("common.delete")}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
...
function AddTriggerDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setLabel("");
      toast.success(t("autopilot.detail.triggerAdded"));
    } catch {
      toast.error(t("autopilot.detail.failedAddTrigger"));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm">
        <DialogTitle>{t("autopilot.detail.addTrigger")}</DialogTitle>
        <div className="space-y-4 pt-2">
          <TriggerConfigSection config={config} onChange={setConfig} />
          <div>
            <label className="text-xs font-medium text-muted-foreground">{t("autopilot.detail.labelOptional")}</label>
            <input
              type="text"
              value={label}
              onChange={(e) => setLabel(e.target.value)}
              placeholder={t("autopilot.detail.labelPlaceholder")}
          ...
          <div className="flex justify-end pt-1">
            <Button size="sm" onClick={handleSubmit} disabled={submitting}>
              {submitting ? t("autopilot.detail.addingTrigger") : t("autopilot.detail.addTrigger")}
            </Button>
          </div>
        </div>
      </DialogContent>
...
export function AutopilotDetailPage({ autopilotId }: { autopilotId: string }) {
  ...
  if (!data) {
    return (
      <div className="flex items-center justify-center h-full text-muted-foreground">
        {t("autopilot.detail.autopilotNotFound")}
      </div>
    );
  }
  ...
  const handleRunNow = async () => {
    try {
      ...
      toast.success(t("autopilot.detail.autopilotTriggered"));
    } catch (e: any) {
      toast.error(e?.message || t("autopilot.detail.failedTrigger"));
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(t("autopilot.detail.autopilotDeleted"));
      router.push(wsPaths.autopilots());
    } catch {
      toast.error(t("autopilot.detail.failedDelete"));
      setDeleting(false);
    }
  };
    ...
    <div className="flex h-full flex-col">
      ...
      <PageHeader className="justify-between px-5">
        <div className="flex items-center gap-2">
          ...
          <div className="ml-1 flex items-center gap-1.5">
            <Switch
              ...
              aria-label={autopilot.status === "active" ? t("autopilot.detail.pauseAutopilot") : t("autopilot.detail.activateAutopilot")}
            />
            <span className={cn(
        ...
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" onClick={() => setEditDialogOpen(true)}>
            ...
            {t("autopilot.detail.edit")}
          </Button>
          <Button size="sm" onClick={handleRunNow} disabled={autopilot.status !== "active" || triggerAutopilot.isPending}>
            <Play className="h-3.5 w-3.5 mr-1" />
            {triggerAutopilot.isPending ? t("autopilot.detail.running") : t("autopilot.detail.runNow")}
          </Button>
        </div>
      </PageHeader>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-4xl mx-auto p-6 space-y-8">
          {/* Properties */}
          <section className="space-y-4">
            <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("autopilot.detail.properties")}</h2>
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <label className="text-xs text-muted-foreground">{t("autopilot.detail.agent")}</label>
              ...
              <div>
                <label className="text-xs text-muted-foreground">{t("autopilot.detail.outputMode")}</label>
                <div className="mt-1">
                  {autopilot.execution_mode === "create_issue" ? t("autopilot.detail.createIssue") : t("autopilot.detail.runOnly")}
                </div>
              </div>
              {autopilot.description && (
                <div className="col-span-2">
                  <label className="text-xs text-muted-foreground">{t("autopilot.detail.prompt")}</label>
                  <div className="mt-1">
          ...
          <section className="space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("autopilot.detail.triggers")}</h2>
              <Button size="sm" variant="outline" onClick={() => setTriggerDialogOpen(true)}>
                <Plus className="h-3.5 w-3.5 mr-1" />
                {t("autopilot.detail.addTrigger")}
              </Button>
            </div>
            {triggers.length === 0 ? (
              <div className="rounded-md border border-dashed p-4 text-center text-sm text-muted-foreground">
                {t("autopilot.detail.noTriggers")}
              </div>
            ) : (
          ...
          {/* Run History */}
          <section className="space-y-3">
            <h2 className="text-sm font-medium text-muted-foreground uppercase tracking-wider">{t("autopilot.detail.runHistory")}</h2>
            {runsLoading ? (
              <div className="space-y-1">
                {Array.from({ length: 3 }).map((_, i) => (
                  <Skeleton key={i} className="h-10 w-full" />
          ...
          <section className="space-y-3 pt-4 border-t">
            <h2 className="text-sm font-medium text-destructive uppercase tracking-wider">{t("autopilot.detail.dangerZone")}</h2>
            <Button size="sm" variant="destructive" onClick={() => setDeleteConfirmOpen(true)}>
              <Trash2 className="h-3.5 w-3.5 mr-1" />
              {t("autopilot.detail.deleteAutopilot")}
            </Button>
          </section>
        </div>
      </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("autopilot.detail.deleteAutopilot")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("autopilot.detail.deleteAutopilotDesc", { title: autopilot.title })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deleting}>{t("autopilot.detail.cancelTrigger")}</AlertDialogCancel>
