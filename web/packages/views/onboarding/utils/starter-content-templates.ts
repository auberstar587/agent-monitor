...
export function buildWelcomeIssueText(
  ...
  const header = [
    ...
    ``,
    `[Learn how Agent Monitor works →](https://multica.ai/docs/how-agent-monitor-works)`,
    ``,
    `---`,
    ``,
  ].join("\n");
  ...
  const sharedInstructions = [
    ...
    `2. **Explain how we work together in Agent Monitor**:`,
  ...
  switch (q.use_case) {
    case "coding":
      return {
        title: "👋 Welcome to Agent Monitor — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Agent Monitor. They plan to use you mostly for **coding work**.\n\n${sharedInstructions}`,
      };
    case "planning":
      return {
        title: "👋 Welcome to Agent Monitor — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Agent Monitor. They want your help with **planning and breaking down work**.\n\n${sharedInstructions}`,
      };
    case "writing_research":
      return {
        title: "👋 Welcome to Agent Monitor — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Agent Monitor. They'll use you for **research and writing** — drafting, summarizing, analysis.\n\n${sharedInstructions}`,
      };
    case "explore":
      return {
        title: "👋 Welcome to Agent Monitor — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Agent Monitor. They're **exploring** what Agent Monitor can do — no specific goal yet.\n\n${exploreInstructions}`,
      };
    case "other": {
      ...
      return {
        title: "👋 Welcome to Agent Monitor — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Agent Monitor. ${contextLine}\n\n${sharedInstructions}`,
      };
    }
    default:
      return {
        title: "👋 Welcome to Agent Monitor — let's work together",
        description: `${header}Hi agent, this is ${name}'s first time using Agent Monitor.\n\n${sharedInstructions}`,
      };
  }
}
...
export function buildAgentGuidedSubIssues(
  ...
  const tier1: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        `**Every issue has a right-side panel** called **Properties**. From there you control who works on what. Agents in Agent Monitor are triggered when an issue has:`,
        ``,
  ...
  if (q.team_size === "team") {
    tier2.push({
      ...
      description: [
        `Agent Monitor works best when a small team shares agents.`,
        ``,
        `**Where to find it**:`,
  ...
  const tier2: ImportStarterIssuePayload[] = [
    {
      ...
      description: [
        ...
        ``,
        `**How Agent Monitor triggers agents**:`,
...
export function buildImportPayload({
  ...
  return {
    ...
    project: {
      ...
      description:
        "A few things to try in Agent Monitor. Work through them at your own pace.",
      icon: "👋",
    },
