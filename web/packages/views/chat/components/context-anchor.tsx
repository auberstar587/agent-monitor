...
export function ContextAnchorButton() {
  ...
  const tooltipText = isDisabled
    ? "Nothing to share with Agent Monitor on this page"
    : focusMode && candidate
      ? candidate.type === "issue"
        ? `Agent Monitor knows you're viewing ${candidate.label} · Click to turn off`
        : `Agent Monitor knows you're viewing project "${candidate.label}" · Click to turn off`
      : "Let Agent Monitor know what you're viewing";
      ...
      <TooltipTrigger
        render={
          <Button
            ...
            disabled={isDisabled}
            aria-label={
              focusMode ? "Stop sharing current page" : "Share current page with Agent Monitor"
            }
...
export function ContextAnchorCard() {
  ...
  const tooltipText =
    candidate.type === "issue"
      ? `Agent Monitor knows you're viewing ${candidate.label}${candidate.subtitle ? ` — ${candidate.subtitle}` : ""}`
      : `Agent Monitor knows you're viewing project "${candidate.label}"`;
