    ...
    return (
      <div className="flex flex-1 min-h-0 flex-col">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-3 px-6 py-16 text-center">
          ...
          <div>
            <p className="text-sm font-medium">{t("agents.error.title")}</p>
            <p className="mt-1 text-xs text-muted-foreground">
              {listError instanceof Error
                ? listError.message
                : t("agents.error.description")}
            </p>
          </div>
          <Button
            type="button"
              ...
              >
                ...
                */}
                ...
                >
                  ...
                  <span className="hidden md:block">{t("agents.table.lastRun")}</span>
                  <span className="hidden md:block">{t("agents.table.runtime")}</span>
                  <span className="hidden lg:block">{t("agents.table.activity")}</span>
                  <span className="hidden text-right md:block">{t("agents.table.runs")}</span>
...
function ActiveToolbarRow({
  ...
  return (
    <div className="flex h-12 shrink-0 items-center gap-2 border-b px-4">
      ...
      <ScopeSegment scope={scope} setScope={setScope} counts={scopeCounts} t={t} />
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="ml-auto flex items-center gap-3">
          {archivedCount > 0 && (
            ...
            >
              {t("agents.showArchived")} ({archivedCount}) →
            </button>
          )}
          <span className="font-mono text-xs tabular-nums text-muted-foreground/70">
  ...
  return (
    <div className="flex h-12 shrink-0 items-center gap-3 border-b px-4">
      ...
      >
        ...
        {t("agents.archived.active")}
      </button>
      <span className="text-muted-foreground/40">/</span>
      <span className="text-xs font-medium">{t("agents.archived.archived")}</span>
      <span className="font-mono text-xs tabular-nums text-muted-foreground/70">
        {archivedCount}
      </span>
      <div className="ml-auto">
...
function EmptyState({ onCreate, t }: { onCreate: () => void; t: (key: string) => string }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <h2 className="mt-4 text-base font-semibold">{t("agents.empty.noAgents")}</h2>
      <p className="mt-1 max-w-md text-sm text-muted-foreground">
        Create an agent and assign it issues, like any teammate. Local agents
        run on your machine; cloud agents run on Agent Monitor&rsquo;s runtime.
      </p>
...
function NoMatches({
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-2 px-4 py-16 text-center text-muted-foreground">
      ...
      <p className="text-sm">{t("agents.empty.noMatches")}</p>
